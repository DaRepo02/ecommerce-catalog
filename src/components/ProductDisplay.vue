...
import UnavailableComponent from './UnavailableComponent.vue'
...
<template>
    <main :class="state.category ? state.category === 'men\'s clothing' ? 'man' : 'woman' : 'empty'">
        ...
        <div class="product-card">
            ...
            <UnavailableComponent @onNext="incrementCount" v-if="!state.isLoading && !state.category" />
            <LoadComponent v-if="state.isLoading" />
        </div>
    </main>
</template>
